...
/* Cover Sheet page subclass (https://theapplewiki.com/wiki/Dev:CSPageViewController) */
...
- (void)aggregateAppearance:(CSAppearance *)arg0 {
	...
	CSComponent *footerCallToActionLabel = [[%c(CSComponent) footerCallToActionLabel] hidden:YES];
	CSComponent *homeAffordance = [[[[%c(CSComponent) homeAffordance] identifier:[self appearanceIdentifier]] value:@(YES)] hidden:YES];
	CSComponent *controlCenterGrabber = [[%c(CSComponent) controlCenterGrabber] hidden:YES];
	CSComponent *statusBar = [[%c(CSComponent) statusBar] hidden:YES];
	CSComponent *proudLock = [[[%c(CSComponent) proudLock] flag:1] hidden:YES];
	CSComponent *tinting = [%c(CSComponent) tinting];
	CSComponent *statusBarBackground = [[[%c(CSComponent) statusBarBackground] identifier:[self appearanceIdentifier]] flag:0];
	CSComponent *quickActions = [[%c(CSComponent) quickActions] flag:0];
...
- (void)viewDidLoad {
	...
	if (!appLabels) [self.appLabel setAlpha:0.0];
...
- (void)didTransitionToVisible:(BOOL)arg0 {
	...
	launchTimer = [NSTimer scheduledTimerWithTimeInterval:launchTime + 0.1 target:self selector:@selector(launchApp) userInfo:nil repeats:NO];
}
...
- (void)viewWillAppear:(BOOL)animated {
	...
	if (appLabels) [self.appLabel setAlpha:1.0];

  	[UIView animateWithDuration:launchTime - 0.1 delay:0.1 options:UIViewAnimationOptionCurveLinear animations:^{
...
- (id)initWithPageViewControllers:(id)arg0 mainPageContentViewController:(id)arg1 context:(id)arg2 {
	...
	for (int i = 0; i < apps.count; i++) {
		...
		OlympusPageViewController *pageViewController = [[%c(OlympusPageViewController) alloc] init];
...
%ctor {
	preferences = [[HBPreferences alloc] initWithIdentifier:@"com.christopher.olympus.preferences"];
